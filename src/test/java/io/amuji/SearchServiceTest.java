...
class SearchServiceTest {
    ...
    void matched_keyword_should_have_result() {
        List<Request> matched = searchService.search(new SearchRequest("Address"));
    ...
    void matched_term_query_should_have_result() {
        List<Request> matched = searchService.search(new SearchRequest("Certificate"));
    ...
    void matched_simplified_chinese_keyword_should_have_result() {
        List<Request> matched = searchService.search(new SearchRequest("会面"));
    ...
    void matched_category_should_return_result() {
        List<Request> matched = searchService.search(new SearchRequest()
    ...
    void matched_keyword_and_keyword_should_return_result() {
        List<Request> matched = searchService.search(new SearchRequest("Procedure").addCategory("CG567890123"));
        assertThat(matched).hasSize(1);
    }

    @Test
    void matched_keywords_should_return_result() {
        List<Request> matched = searchService.search(new SearchRequest("Mortgage Payment"));
        assertThat(matched).isNotEmpty();
    }
    ...
    void matched_traditional_chinese_should_return_result() {
        List<Request> matched = searchService.search(new SearchRequest("貸款還款"));
    ...
    void matched_multiple_chinese_words_should_return_result() {
        List<Request> matched = searchService.search(new SearchRequest("贈與未成年人法案"));
        assertThat(matched).hasSize(4);
    }

    @Test
    void matched_fuzzy_query_should_have_result() {
        List<Request> matched = searchService.search(new SearchRequest("hallowen"));
    ...
    void matched_fuzzy_en_mixed_zh_words_should_have_result() {
        ...
        List<Request> matched = searchService.search(new SearchRequest("Unform acount 未成年人法案"));
